...
// // // spiritual // // //
app.get("/spirituality", (request, response) => {
	const query = `SELECT * FROM spirituality LIMIT 4`;
	//Execute Query
	db.query(query, (error, result) => {
		...
		response.render('spiritual.pug', { products : result, cart : request.session.cart });
	});
});

// add to card function to shopping card // // spiritual
app.post('/add_cart', (request, response) => {
	...
	response.redirect("/spirituality");
});

// remove from shopping card function // // spiritual
...
app.get("/motivation", (request, response) => {
	const query = `SELECT * FROM motivation LIMIT 4`;
	//Execute Query
	db.query(query, (error, result) => {
		if(!request.session.cart)
		{
		...
		response.render('motivation.pug', { products : result, cart : request.session.cart });
	});
});

// add to card function to shopping card // // spiritual
app.post('/add_cart_motivation', (request, response) => {
	const product_id = request.body.product_id;
	const product_name = request.body.product_name;
	const product_price = request.body.product_price;
	let count = 0;
	...
	response.redirect("/motivation");
});

// remove from shopping card function // // spiritual
app.get('/remove_item_motivation', (request, response) => {
